...
with patch("engine.data_downloader.yf") as mock_yf, \
    ...
    print(f"  Naive continuous:  X={X_cont.shape}, y range=[{y_cont.min():.4f}, {y_cont.max():.4f}]")
    ...
    for tw in [False, True]:
        label = "Time-Weighted" if tw else "Standard"
        cfg = PredictionConfig(ticker="TEST", period="1y", model_type="knn_enhanced",
                               use_time_weights=tw, include_news=False)
        result = api.get_prediction(cfg)
        print(f"  {label}: {result.prediction} {result.confidence} ({result.data_points} pts)")
        assert result.prediction in ("UP", "DOWN")
    ...
    for tw in [False, True]:
        label = "Time-Weighted" if tw else "Standard"
        cfg = PredictionConfig(ticker="TEST", period="1y", model_type="linreg",
                               use_time_weights=tw, include_news=False)
        result = api.get_prediction(cfg)
    ...
    for tw in [False, True]:
        label = "Time-Weighted" if tw else "Standard"
        cfg = PredictionConfig(ticker="TEST", period="1y", model_type="linreg_enhanced",
                               use_time_weights=tw, include_news=False)
        result = api.get_prediction(cfg)
        print(f"  {label}: {result.prediction} {result.confidence}")
        assert result.prediction in ("UP", "DOWN")
    ...
    for feats in feature_combos:
        ...
        print(f"  {label:<35}  k-NN={knn_pred:<5} {knn_prob:.0%}  LinReg={lr_pred:<5} {lr_prob:.0%}")
    ...
    if scraper.vader_available:
        ...
        print(f"  VADER available: no (using naive fallback)")
    ...
    for model, period in news_test_periods.items():
        cfg_news = PredictionConfig(ticker="TEST", period=period, model_type=model,
                                    use_time_weights=True, include_news=True)
        res_news = api.get_prediction(cfg_news)
        print(f"  {model:<18} ({period}): {res_news.prediction} {res_news.confidence} "
    ...
    for p in ["1mo", "1y", "2y", "max"]:
        for model in ["knn", "knn_enhanced", "linreg", "linreg_enhanced"]:
            try:
                cfg = PredictionConfig(ticker="TEST", period=p, model_type=model,
                                       use_time_weights=False, include_news=False)
                r = api.get_prediction(cfg)
                print(f"  {p:<6} {model:<18} {r.prediction:<6} {r.confidence:<8} ({r.data_points} pts)")
            except RuntimeError:
    ...
    for model, name in models_to_test:
        ...
        print(f"  {name:<18}: {bt.correct}/{bt.test_days} ({bt.accuracy:.0%})  "
    ...
    try:
        ...
        print(f"  Invalid model_type: caught ValueError ✓")
    ...
    try:
        ...
        print(f"  Invalid k-NN feature: caught ValueError ✓")
    ...
    try:
        ...
        print(f"  Invalid LinReg feature: caught ValueError ✓")
    ...
    try:
        ...
        if TORCH_AVAILABLE:
            ...
            # Cleanup
            test_path.unlink()
            print(f"  LSTM with PyTorch: all checks passed ✓")
        else:
            print(f"  PyTorch not installed — testing graceful fallback")
            ...
            try:
                ...
                print(f"  LSTM not available: caught RuntimeError ✓")

    except ImportError:
        print(f"  ai_model.py import failed — skipping LSTM tests")
        assert not api.lstm_available
        print(f"  api.lstm_available = False ✓")
